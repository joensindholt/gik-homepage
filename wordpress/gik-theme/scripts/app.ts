      ...
      var eventsApp = new Vue({
        ...
        methods: {
          ...
          prettyLastRegistrationDate: function(event) {
            return moment(event.registrationPeriodEndDate).format(
              "D. MMMM YYYY"
            );
          },
          ...
          isOpenForRegistration: function(event) {
            var now = new Date();
            var registrationPeriodEndDateOffset = new Date(
              event.registrationPeriodEndDate
            );
            registrationPeriodEndDateOffset.setDate(
              registrationPeriodEndDateOffset.getDate() + 1
            );
            return (
              new Date(event.registrationPeriodStartDate) <= now &&
...
app.dateService = {
  ...
  parseDateAsCopenhagenTime: dateString => {
    ...
    return new Date(
      dateString + "+" + app.dateService.getTimezoneOffsetString()
    );
  },
